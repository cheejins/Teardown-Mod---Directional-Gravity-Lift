...
function tick()
    ...
    -- local gvDim = CalcDistance(gv.startPos, gv.endPos)

    local x = 10
    local y = 10
    local z = 10

    worldShapesList = QueryAabbShapes(Vec(-x,-y,-z), Vec(x,y,z))
    ...
    for i=1, #gravlistObjectList do
        ...
        local gravlift = gravlistObjectList[i]
        runGravlift(gravlift)
        drawGvOutline(gravlift)
    end
end
...
function runGravlift(gv)
    ...
    local vectorLineRadius = gv.radius * 1.2

    local x1 = gv.startPos[1] + vectorLineRadius
    local y1 = gv.startPos[2] - vectorLineRadius
    local z1 = gv.startPos[3] + vectorLineRadius

    local x2 = gv.endPos[1] - vectorLineRadius
    local y2 = gv.endPos[2] + vectorLineRadius
    local z2 = gv.endPos[3] - vectorLineRadius
    ...
    gravliftAabbShapesList = QueryAabbShapes(aabbStart, aabbEnd)

    for i=1, #gravliftAabbShapesList do

        local shape = gravliftAabbShapesList[i]
