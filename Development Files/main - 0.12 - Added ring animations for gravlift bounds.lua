...
function buildDefaultGravliftObject(body)
    ...
    --dimensions:
        ...
        -- _gv.radius = GetTagValue(body, "radius")
        _gv.radius = 2
    ...
    --sound:
        ...
        _gv.sound_enter_vol = GetTagValue(body, "sound_enter_volume")/2
        _gv.sound_loop = gv_sounds.gv_loop
        _gv.sound_loop_vol = GetTagValue(body, "sound_loop_volume")/2
...
function runGravlift(gv)
    ...
    for i=1, #shapesList do
        ...
        if withinCylinder(gv.startPos, gv.endPos, gv.radius, shapeTransform.pos) then
            ...
            -- local additiveVelocity = VecAdd(
            --     VecScale(GetBodyVelocity(shape),1), 
            --     VecScale(gv.getDirectionalVelocity(), 2))
            ...
            -- SetBodyAngularVelocity(GetShapeBody(shape), additiveVelocity) -- additive velocity
            -- ApplyBodyImpulse(GetShapeBody(shape), gv.startPos, gv.getDirectionalVelocity())


            PlayLoop(gv.sound_loop, gv.startPos, gv.sound_loop_vol)
...
function gravliftAnimation(gv)
    ...
    -- for i=1, 10 do
    --     -- gv length for tr
    --     timeModulus =  math.fmod(
    --         GetTime()*gv.length, 
    --         gv.length)/gv.length * i

    --     lerpPos = VecLerp(gv.startPos, gv.endPos, timeModulus)
    ...
    --         local endPoint = TransformToParentPoint(tr_g, Vec(0,0,-gv.length/2))
